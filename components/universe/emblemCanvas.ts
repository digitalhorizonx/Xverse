...
/**
 * Renders a product's glowing emblem — outer ring plus glyph — to a raw
 * canvas. Two passes per element: a heavy blurred stroke in the product
 * color for the glow body, then a thin near-white core so the mark reads
 * hot and crisp. Black background disappears under additive blending in
 * the 3D scene; the DOM <EmblemImage> uses the transparent PNG as-is.
 *
 * Deliberately free of any three.js import: DOM consumers (showcase pages)
 * must not pay for the 3D bundle. The texture wrapper lives in emblems.ts.
 */
export function drawEmblemCanvas(
